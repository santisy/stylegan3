...
class MOVQDecoder(nn.Module):
    def __init__(
        ...
        no_atten_decoder=False,
        **ignorekwargs
    ):
        super().__init__()
        self.temb_ch = 0
        self.num_resolutions = int(np.log2(resolution // res_in)) + 1
        self.num_res_blocks = num_res_blocks
        self.resolution = resolution
        self.give_pre_end = give_pre_end
        self.movq_stylelike = movq_stylelike
        self.no_atten_decoder = no_atten_decoder
        ch_mult = ch_mult[::-1]
        assert(len(ch_mult) == self.num_resolutions)
        ...
        for i_level in range(self.num_resolutions):
            ...
            for _ in range(self.num_res_blocks):
                ...
                if attn_resolutions is not None and curr_res in attn_resolutions:
                    attn.append(AttnBlock(block_in, z_channels, add_conv=add_conv))
            up = nn.Module()
            up.block = block
            up.attn = attn
            self.levels_list[i_level] += 1
        ...
        # end
        self.norm_out = Normalize(block_in, z_channels, add_conv=add_conv)
    ...
    def forward(self, z, zq_list):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            for i_block in range(self.num_res_blocks):
                h = self.up[i_level].block[i_block](h, temb, z_now)
                if len(self.up[i_level].attn) > 0:
                    h = self.up[i_level].attn[i_block](h, z_now)
            if i_level != 0:
